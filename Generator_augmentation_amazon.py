...
def image_augmentation(img, ang_range=6, shear_range=3, trans_range=3):
    ...
    random_bright = .8 + np.random.uniform()
    ...
    # Blur
    blur_value = random.randint(0,2) * 2 + 1
...
class ImageGenerator:
    ...
    def ver2006(self, num, save=False):
        ...
        for i, Iter in enumerate(range(num)):
            ...
            cv2.rectangle(background, (0, 0), (random_width, random_height), (0, 0, 0), -1)
            ...
            backgroundAmazon=cv2.imread("background/"+imgName + ".jpg")
            backgroundAmazon = cv2.resize(backgroundAmazon, (260, 55))
            ...
            backgroundAmazon = cv2.add(background, backgroundAmazon)

            if save:
                cv2.imwrite(self.save_path + "lastDB/" + str(cnt) + ".jpg", backgroundAmazon)
